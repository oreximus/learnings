...
- Provided with `capture.pcapng` file which contains the data packets
- Which helps us to get the information about `/freediscordnitro`:

![](imgs/01_fakeboost.png)

- Which contains some powershell script generates `nitro codes`
- Then it contains hint about getting the whole base64 encoded script content!
...
- This contains a function which `encrypt the data using AES key` and in the mentioned that POST URL was used `http://192.168.116.135:8080/rj1893rj1joijdkajwdal`
- In the wireshark we also find the encrypted message!

![](imgs/02_fakeboost.png)

- From the reference of Encryption Function in the powershell, we can create a Decryption function and Decrypt the data.
